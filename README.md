...
2. Create DB and set `DATABASE_URL` in `.env`
...
## Complaints (current design)

The app uses a minimal complaint model. The server requires only `complaint_type`; an optional `description` may be provided for context.
...
- `description` was added as an optional column

---

## API Quick Reference (Postman-ready)

Base URL: `http://localhost:3000`

Authentication: add header `Authorization: Bearer <token>` for protected routes.
...
3) Raise Ticket (required: `complaint_type`; optional: `description`)

POST `/complaints/raise-ticket`
...
```
{ "complaint_type": "live_demo", "description": "Request demo for enterprise features" }
```

```
{ "complaint_type": "billing_issue", "description": "Charged twice for Jan subscription" }
```
...
4) List complaints (filters: `status`, `complaint_type`, pagination)

GET `/complaints?complaint_type=technical_issue&status=raised&page=1&limit=10`
